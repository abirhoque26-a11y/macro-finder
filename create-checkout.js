...
module.exports = async (req, res) => {
  ...
  try {
    ...
    /* Has this account already had its free trial?

       Stripe grants a trial on every subscription it is asked to, so without
       this check someone could subscribe, cancel on day two, resubscribe, and
       repeat forever without ever being charged. We record trial_end when the
       webhook writes the subscription, so its presence means the trial is
       already spent.

       Reusing the stored stripe_customer_id also stops a second Stripe customer
       being created for the same person, which would split their billing
       history across two records. */
    let alreadyTrialled = false;
    let existingCustomer = null;
    try {
      const KEY = env('SUPABASE_SERVICE_ROLE_KEY');
      if (KEY) {
        const q = await fetch(
          `${SUPABASE_URL}/rest/v1/subscriptions?user_id=eq.${encodeURIComponent(user.id)}&select=trial_end,stripe_customer_id`,
          { headers: { apikey: KEY, Authorization: `Bearer ${KEY}` } }
        );
        const rows = await q.json();
        if (Array.isArray(rows) && rows[0]) {
          alreadyTrialled = !!rows[0].trial_end;
          existingCustomer = rows[0].stripe_customer_id || null;
        }
      }
    } catch (e) {
      // If the lookup fails we fall through and grant a trial. Better to give
      // away three days than to block a genuine customer from subscribing.
      console.error('trial-eligibility lookup failed', e.message);
    }

    const site = SITE_URL || `https://${req.headers.host}`;
    const subData = { metadata: { supabase_user_id: user.id } };
    if (!alreadyTrialled) subData.trial_period_days = TRIAL_DAYS;

    const payload = form({
      mode: 'subscription',
      client_reference_id: user.id,
      // an existing Stripe customer must be referenced by id, not by email
    ...
    return res.status(200).json({ url: session.url, trial: !alreadyTrialled });
  } catch (err) {
